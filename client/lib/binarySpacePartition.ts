...
export class BinarySpacePartition {
  ...
  private _branchesOperation?: (branch_a: Container, branch_b: Container) => void = () => {};
  ...
  initializeTree() {
    const container = new Container(0, 0, 0, this._mapWidth, this._mapLevelHeight, this._mapDepth);
    this.generateTree(container, this._iterations, 'root');
  }

  generateTree(container: Container, iterations: number, branch?: Branch): TreeNode<Container> {
    const node = new TreeNode<Container>(container, this._iterations - iterations, branch);
    const sizeBuffer = this._minSize * 2;
    if (iterations !== 0 && node._node._width > sizeBuffer && node._node._depth > sizeBuffer) {
      ...
      this._branchesOperation && this._branchesOperation(node._branch_a._node, node._branch_b._node);
    } else {
