import { Color3, LinesMesh, MeshBuilder, Scene, TransformNode, Vector3 } from '@babylonjs/core';
...
export class Triangle extends TransformNode {
  ...
  shortestSide() {
    const shortest = this._lines.reduce((acc: Line, current: Line) => {
      if (Vector3.Distance(current.vectors[0], current.vectors[1]) < Vector3.Distance(acc.vectors[0], acc.vectors[1])) {
        acc = current;
      }
      return acc;
    }, this._lines[0]);
