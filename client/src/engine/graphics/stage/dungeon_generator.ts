...
export class DungeonGenerator extends TransformNode {
  ...
  generateRoom({ _x: x, _y: y, _z: z }: Container) {
    const randomizedWidth = random(this._minRoomSize * 0.5, this._minRoomSize * 0.8);
    const randomizedDepth = random(this._minRoomSize * 0.5, this._minRoomSize * 0.8);
    const room = new Room({
  ...
  generateCorridor(branch_a: Container, branch_b: Container) {
    ...
    const room_a = this._rooms[`room_x${branch_a._x}_y${branch_a._y}_z${branch_a._z}`];
    const room_a_depth = room_a ? room_a.depth : 0;
    const room_b = this._rooms[`room_x${branch_b._x}_y${branch_b._y}_z${branch_b._z}`];
