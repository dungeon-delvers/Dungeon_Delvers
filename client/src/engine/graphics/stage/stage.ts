...
export class Stage {
  ...
  public async load() {
    const dungeonAssetsResult = await SceneLoader.ImportMeshAsync(null, '', dungeonAssets, this._scene);
    this._assets = dungeonAssetsResult.meshes.reduce((accumulator: StageAssets, asset: AbstractMesh) => {
