...
export class AttributeSelect {
  ...
  constructor(menuId: string, attributes: Attributes) {
    ...
    Object.values(ATTRIBUTES).forEach(attribute => {
      ...
      attributeGrid.addRowDefinition(0.5);
      const attributeValueGrid = new Grid(`${this._menuId}__attribute_${attribute}_values_grid`);
      attributeValueGrid.width = '160px';
      attributeValueGrid.height = '40px';
      attributeValueGrid.addColumnDefinition(0.33);
      attributeValueGrid.addColumnDefinition(0.33);
      attributeValueGrid.addColumnDefinition(0.33);
      const attributeLabel = new TextBlock(`${this._menuId}__attribute_${attribute}`, attribute);
      this._attributeValues[attribute as ATTRIBUTES].text = this._attributes.getAttribute(attribute).value.toString();
  ...
  updateAtribute(availablePoints: number, attribute: ATTRIBUTES, value: number) {
  ...
  updateAttributes(attributes: Attributes) {
    ...
    this._availablePointsLabel.text = `Available Points: ${this._availablePoints}`;
    Object.values(ATTRIBUTES).forEach(attribute => {
      this._attributeValues[attribute].text = this._attributes.getAttribute(attribute).value.toString();
    });
  }
}
