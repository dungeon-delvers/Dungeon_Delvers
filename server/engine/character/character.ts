...
import { ATTRIBUTES, ATTRIBUTE_MAX_CREATION_VALUE, ATTRIBUTE_MIN_CREATION_VALUE } from './attributes';
...
export const create = async ({ name, archtype, attributes: inAttributes }: CharacterCreationArgs) => {
  const attributes = {
  ...
  const stats = {
    [STATS.ACCURACY]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.PER], BASE_MODIFIERS[STATS.ACCURACY]),
      archtypes[archtype].BASE_STATS[STATS.ACCURACY],
    ),
    [STATS.ACTION_SPEED]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.DEX], BASE_MODIFIERS[STATS.ACTION_SPEED]),
    ),
    [STATS.AREA_OF_EFFECT]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.INT], BASE_MODIFIERS[STATS.AREA_OF_EFFECT]),
    ),
    [STATS.CONCENTRATION]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.RES], BASE_MODIFIERS[STATS.CONCENTRATION]),
    ),
    [STATS.DAMAGE]: calculateStat(calculateModifier(attributes[ATTRIBUTES.MIG], BASE_MODIFIERS[STATS.DAMAGE])),
    [STATS.DEFLECTION]: calculateStat(calculateModifier(attributes[ATTRIBUTES.RES], BASE_MODIFIERS[STATS.DEFLECTION])),
    [STATS.DURATION]: calculateStat(calculateModifier(attributes[ATTRIBUTES.INT], BASE_MODIFIERS[STATS.DURATION])),
    [STATS.FORTITUDE]: calculateStat(calculateModifier(attributes[ATTRIBUTES.CON], BASE_MODIFIERS[STATS.FORTITUDE])),
    [STATS.HEALING]: calculateStat(calculateModifier(attributes[ATTRIBUTES.MIG], BASE_MODIFIERS[STATS.HEALING])),
    [STATS.HEALTH]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.CON], BASE_MODIFIERS[STATS.HEALTH]),
      archtypes[archtype].BASE_STATS[STATS.HEALTH],
    ),
    [STATS.REFLEX]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.PER], BASE_MODIFIERS[STATS.REFLEX]) +
        calculateModifier(attributes[ATTRIBUTES.DEX], BASE_MODIFIERS[STATS.REFLEX]),
    ),
    [STATS.WILLPOWER]: calculateStat(
      calculateModifier(attributes[ATTRIBUTES.RES], BASE_MODIFIERS[STATS.WILLPOWER]) +
        calculateModifier(attributes[ATTRIBUTES.INT], BASE_MODIFIERS[STATS.WILLPOWER]),
    ),
  };
...
export default (_io: Server, socket: Socket) => {
  socket.on('character:create', async (args: CharacterCreationArgs, callback) => {
